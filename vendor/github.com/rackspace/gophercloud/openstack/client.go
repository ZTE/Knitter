...
// for example.
func GetDcid(endpoint string) string {
	strs := strings.Split(endpoint, "/")
	//Dcid before 'v2.0' and after 'host:port'
	//klog.Errorf("StrS=[%v]len[%d] ", strs, len(strs))
	if strs[3] == "v2.0" {
		return ""
	}
	if strs[4] == "v2.0" {
...
func NewClient(endpoint string) (*gophercloud.ProviderClient, error) {
	...
	//Change by M11 for vDirector
	base += GetDcid(endpoint)
...
func v2auth(client *gophercloud.ProviderClient, endpoint string, options gophercloud.AuthOptions) error {
	...
	client.TokenIssuedAtLocal = time.Now()
	client.TokenID = token.ID
	client.TenantID = token.Tenant.ID
	client.TenantName = token.Tenant.Name
	client.TokenExpiresAt = token.ExpiresAt
	client.TokenIssuedAt = token.IssuedAt
