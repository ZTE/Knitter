...
func (bulkPorts BulkPorts) ToPortCreateMap() (map[string]interface{}, error) {
	q := make([]interface{}, 0)
	for _, opt := range bulkPorts.Opts {
		p := make(map[string]interface{})
		if opt.NetworkID == "" {
			return nil, errNetworkIDRequired
		}
		p["network_id"] = opt.NetworkID

		if opt.DeviceID != "" {
			p["device_id"] = opt.DeviceID
		}
		if opt.DeviceOwner != "" {
			p["device_owner"] = opt.DeviceOwner
		}
		if opt.FixedIPs != nil {
			p["fixed_ips"] = opt.FixedIPs
		}
		if opt.SecurityGroups != nil {
			p["security_groups"] = opt.SecurityGroups
		}
		if opt.TenantID != "" {
			p["tenant_id"] = opt.TenantID
		}
		if opt.AdminStateUp != nil {
			p["admin_state_up"] = &opt.AdminStateUp
		}
		if opt.Name != "" {
			p["name"] = opt.Name
		}
		if opt.MACAddress != "" {
			p["mac_address"] = opt.MACAddress
		}
		if opt.AllowedAddressPairs != nil {
			p["allowed_address_pairs"] = opt.AllowedAddressPairs
		}
		if opt.VnicType != "" {
			p["binding:vnic_type"] = opt.VnicType
		}
		q = append(q, p)
	}
	return map[string]interface{}{"ports": q}, nil
}
...
func CreateBulk(c *gophercloud.ServiceClient, opts CreateOptsBuilder) CreateResult {
	var res CreateResult
	q, err := opts.ToPortCreateMap()
	if err != nil {
		res.Err = err
		return res
	}
	_, res.Err = c.Post(createURL(c), q, &res.Body, nil)
