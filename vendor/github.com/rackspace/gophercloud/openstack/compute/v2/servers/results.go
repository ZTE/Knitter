...
type interfaceAttachmentResult struct {
	gophercloud.Result
}

// yzh attach nic device to vm - parase response data to json
func (r interfaceAttachmentResult) ExtractInterface() (*InterfaceAttachment, error) {
	if r.Err != nil {
		fmt.Errorf("ExtractInterface: http return error: %s", r.Err)
		return nil, r.Err
	}

	var response struct {
		interfaceAttachment InterfaceAttachment `mapstructure:"interfaceAttachment"`
	}
	...
	decoder, err := mapstructure.NewDecoder(config)
	if err != nil {
		fmt.Errorf("ExtractInterface: NewDecoder error: %s", err)
		return nil, err
	}

	err = decoder.Decode(r.Body)
	if err != nil {
		fmt.Errorf("ExtractInterface: Decode body:[%v] error: %s", r.Body, err)
		return nil, err
	}

	return &response.interfaceAttachment, nil
}
// yzh attach nic device to vm
type AttachResult struct {
	interfaceAttachmentResult
}
// yzh attach nic device to vm - after post request get the reponse data
type InterfaceAttachment struct {
	...
	PortState string `mapstructure:"port_state"`
	FixedIps []interface{}
}
//-----------------------------------2016-03-22------detach--------------