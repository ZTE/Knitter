...
//-----------------------------------2016-03-22------attach--------------
// yzh attach nic device to vm
type AttachOptsBuilder interface {
	ToServerAttachMap() map[string]interface{}
}
...
// yzh attach nic device to vm
func (opts AttachOpts) ToServerUpdateMap() map[string]interface{} {
	server := make(map[string]string)
	if opts.PortID != "" {
		server["name"] = opts.PortID
	}
	return map[string]interface{}{"server": server}
}
// yzh attach nic device to vm
func Attach(client *gophercloud.ServiceClient, id string, opts AttachOptsBuilder) AttachResult {
	...
	reqBody := opts.ToServerAttachMap()
	_, result.Err = client.Post(attachURL(client, id), reqBody, &result.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200},
	})
	return result
}

func (opts AttachOpts) ToServerAttachMap() map[string]interface{} {
	interfaceAttachment := make(map[string]string)
	if opts.PortID != "" {
		interfaceAttachment["port_id"] = opts.PortID
	}

	return map[string]interface{}{"interfaceAttachment": interfaceAttachment}
}

//-----------------------------------2016-03-22------detach--------------
func Detach(client *gophercloud.ServiceClient, server_id string, port_id string) DeleteResult {
	var res DeleteResult
	_, res.Err = client.Delete(detachURL(client, server_id, port_id), nil)
	return res
}
